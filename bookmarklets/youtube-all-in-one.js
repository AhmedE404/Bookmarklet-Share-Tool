(() => {
    window.cmb || (cmb = 1, (f = () => {
        if (document.querySelector('.ad-showing')) {
            movie_player.loadVideoByPlayerVars({ videoId: movie_player.getVideoData().video_id, start: movie_player.getCurrentTime() });
            console.log('تم تخطي الإعلان');
        } else {
            console.log('لا يوجد إعلان');
        }
    })(), new MutationObserver(f).observe(document.querySelector('#movie_player video.html5-main-video'), { attributes: true, attributeFilter: ['src'] }),
        (d => {
